...
function Chatbot() {
  ...
  const [isOpen, setIsOpen] = useState(false); // Start closed for both modes
  ...
  const isStandalonePage =
    location.pathname === "/student-chat" || location.pathname === "/ai-chat";
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const botMessage = { 
        ...
        courses: response.data.courses || [] // Include course context if provided
      };
  ...
  if (isStandalonePage) {
    return (
      <div className="relative flex h-[100dvh] flex-col bg-gradient-to-br from-gray-950 to-gray-900 fixed inset-0 z-50">
        ...
        {isOpen && (
          <>
            {/* Header — shrink-0 on actions so Close is never squeezed off-screen */}
            <div className="flex items-center gap-2 border-b border-cyan-500/20 bg-gradient-to-r from-gray-900 to-gray-800 px-safe py-3 shadow-lg sm:px-4">
              <button
                type="button"
                onClick={() => navigate("/student-dashboard")}
                className="shrink-0 rounded-lg border border-gray-600 bg-gray-800/90 px-2 py-2 text-xs sm:text-sm font-medium text-cyan-300 hover:bg-gray-700 hover:border-cyan-500/50"
              >
                Back
              </button>
              <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
                <MessageSquare className="h-6 w-6 shrink-0 text-cyan-400 sm:h-8 sm:w-8" />
                <h1 className="truncate text-base font-bold text-gray-100 sm:text-2xl">Edustack AI Chat</h1>
              </div>
              <button
                type="button"
                onClick={toggleChatbot}
              ...
              <div ref={chatContainerRef} className="flex-1 p-3 sm:p-6 overflow-y-auto space-y-3 sm:space-y-5 bg-gray-900/50 rounded-xl border border-gray-700/50 mb-3 sm:mb-4" style={{ maxHeight: "calc(100vh - 180px)" }}>
                {messages.length === 0 ? (
                  <div className="text-center text-gray-200 mt-8 sm:mt-14">
                    <p className="text-base sm:text-xl font-semibold animate-pulse-slow">Welcome to Edustack AI</p>
                    <p className="text-xs sm:text-sm mt-2 text-gray-300 opacity-80">Ask anything to brighten your day!</p>
                  </div>
                ) : (
  ...
  // Floating Widget Mode — full-width panel on small screens, safe-area inset for notched phones
  return (
          ...
          <div className="flex items-center gap-2 border-b border-gray-600/50 bg-gray-900/90 p-2 sm:p-4 backdrop-blur-md">
            <div className="flex min-w-0 flex-1 items-center gap-2">
              ...
              <h3 className="truncate text-sm font-bold text-cyan-100 sm:text-xl">
                <span className="hidden sm:inline">Edustack AI</span>
                <span className="sm:hidden">Edustack</span>
              </h3>
            </div>
            <button
              type="button"
          ...
          <div ref={chatContainerRef} className="flex-1 p-3 sm:p-6 overflow-y-auto space-y-3 sm:space-y-5 bg-gray-900/80 backdrop-blur-sm">
            {messages.length === 0 ? (
              <div className="text-center text-gray-200 mt-14">
                <p className="text-base font-semibold animate-pulse-slow">Welcome to Edustack AI</p>
